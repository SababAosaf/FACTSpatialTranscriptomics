...
class Encoder(Module):
    def __init__(self, in_features, out_features ,dropout=0.0, act=F.relu):
        ...
        # self.weight2 = Parameter(torch.FloatTensor(512, 256))
        # self.weight3_ = Parameter(torch.FloatTensor(256, 128))
        # self.weight4_ = Parameter(torch.FloatTensor(128, 256))
        # self.weight5 = Parameter(torch.FloatTensor(256, 512))
        # self.weight6  = Parameter(torch.FloatTensor(512, self.in_features))
        self.weight3 = Parameter(torch.FloatTensor(self.out_features, 9))
        self.weight2 = Parameter(torch.FloatTensor(self.out_features, self.in_features))
        # self.spatialxw = Parameter(torch.FloatTensor(self.out_features,self.spatialx))
        # self.spatialyw = Parameter(torch.FloatTensor(self.out_features,self.spatialy))
    ...
    def reset_parameters(self):
        ...
        # torch.nn.init.xavier_uniform_(self.weight5)
        # torch.nn.init.xavier_uniform_(self.weight6)
        # torch.nn.init.xavier_uniform_(self.weight4_)
        # torch.nn.init.xavier_uniform_(self.weight3_)
        # torch.nn.init.xavier_uniform_(self.spatialxw)
        # torch.nn.init.xavier_uniform_(self.spatialyw)
    ...
    def forward(self, feat, feat_a):
        ...
        # h1 = torch.mm(h, self.weight3_)
        # h2 = torch.mm(h1, self.weight4_)
        # h3 = torch.mm(h2, self.weight5)
        # h4 = torch.mm(h3, self.weight6)
        # spx = torch.mm(z, self.spatialxw)
        # spy = torch.mm(z, self.spatialyw)
        labels = torch.mm(z, self.weight3)
        return hiden_emb, h ,labels
