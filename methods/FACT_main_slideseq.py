...
import methods.ACT_Network_slideseq as ACT_Network
...
def FACT(database):
    ...
    for i in files:
        ...
        dataset='E:\Project_Large_Datasets\ST\Stereoseq'
        ...
        if Filtering:
            ...
            count_cut = np.quantile(adata.obs["total_counts"], 0.75)  # keep top 60%
            gene_cut = np.quantile(adata.obs["n_genes_by_counts"], 0.75)
        ...
        if tool == 'mclust':
           ...
           values = clustering(adata, n_clusters, radius=radius, method=tool, refinement=False)
