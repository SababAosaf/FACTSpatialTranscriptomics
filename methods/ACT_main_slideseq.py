...
import methods.ACT_Network_slideseq
...
def ACT(database):
    ...
    ground_truth = 'layer_guess'
    database='E:\Project_Large_Datasets\ST'
    if database=='DLPFC':
        ground_truth = 'layer_guess'
    else:
    ...
    for i in files:
        ...
        adata = sc.read_h5ad(dataset+'\\FB2_D1_stereo-seq_processed_updated.h5ad')  # OR sc.read_10x_mtx(...), OR sc.read_h5ad(...)
        ...
        if Filtering:
            ...
            # Visualize distributions to pick cutoffs
            sc.pl.violin(adata, ["total_counts", "n_genes_by_counts", "pct_counts_MT"], jitter=0.4, multi_panel=True)

            # Filter: remove background (low counts/genes) + high MT
            count_cut = np.quantile(adata.obs["total_counts"], 0.85)  # keep top 60%
            gene_cut = np.quantile(adata.obs["n_genes_by_counts"], 0.85)

            adata = adata[
                (adata.obs["total_counts"] >= count_cut) &
                (adata.obs["n_genes_by_counts"] >= gene_cut)
                ].copy()

            print("Cuts:", count_cut, gene_cut)
            print("Filtered spots:", adata.n_obs, "out of", adata.n_obs)
        ...
        sq.pl.spatial_scatter(
            ...
            size=1,
            shape=None,
            save="D:\Projects\GraphST\FACT_Spatial_Transcriptomics\cluster_spatial_resu.png"
        )
        exit(1)
        ...
        #     size=1,
        #     shape=None,
        #     save="D:\Projects\GraphST\FACT_Spatial_Transcriptomics\cluster_spatial.png"
        # )
        # exit(1)
        # then read coords and attach
        # coords_df indexed by barcode with columns x,y
        # adata = adata[coords_df.index].copy()  # align barcodes if database!='DLPFC':
        ...
        adata.obsm['emb']=  adata.obsm['feat']
        model = methods.ACT_Network_slideseq.Embedding_Network(adata,d_small, distance_matrix,device=device)
        ...
        if tool == 'mclust':
           ...
           values = clustering(adata, n_clusters, radius=10, method=tool, refinement=False)
        ...
        if values == "CORRECT":
            ...
            ARI = metrics.adjusted_rand_score(adata.obs['domain'], adata.obs['annotation'])
            ...
            sq.pl.spatial_scatter(
                ...
                size=1,
                shape=None,
                save="D:\Projects\GraphST\FACT_Spatial_Transcriptomics\cluster_spatial_results.png"
            )
            ...
            # hom=metrics.homogeneity_score(adata.obs['domain'], adata.obs['cluster'])
            # cm=metrics.completeness_score(adata.obs['domain'], adata.obs['cluster'])
