        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            if epoch > 1 and epoch % 200 == 0:
                ...
                with torch.no_grad():
                    ...
                    totals=30000*32
                    for i in range(0,int(totals*.5)):
                        a=random.randrange(0,30000)
                        b=random.randrange(0,32)
                        p=random.choice([0,1])
                        if p==0:
                            self.model.weight1[a][b]=self.model.weight1[a][b]+self.model.weight1[a][b]*0.01
                        else:
                           self.model.weight1[a][b] = self.model.weight1[a][b] - self.model.weight1[a][b] * 0.01
            ...
            self.hiden_feat, self.emb,self.labels= self.model(self.features, self.features_a, self.adj)
            ...
            # print("centroid")
            # print(centroids)
            # if epoch==1 or epoch%67==0:
            #     self.emb_rec = self.model(self.features, self.features_a, self.adj)[0].detach().cpu().numpy()
            #     self.adata.obsm['emb'] = self.emb_rec
            #     clustering(self.adata, 7, radius=50, method='mclust', refinement=True)#,start=0.1, end=0.5, increment=0.01)
            #     clusr = ch.compute_ASW( self.adata, 'domain')
            # self.clustering = F.softmax(self.labels, dim=1)
