...
import random
import numpy as np
from .autoencoder_model_cluster_slideseq import Encoder, Encoder_sparse, Encoder_map, Encoder_sc
...
class Embedding_Network():
    ...
    def train(self):
        ...
        data, _ = make_blobs(n_samples=self.features.shape[0], centers=n_clusters, cluster_std=0.60, random_state=0)
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            if   epoch >= ONE_THOUSAND and epoch<TWO_THOUSAND:
                ...
                if epoch%(int(ONE_THOUSAND/10))==0:
                    ...
                    clustering(self.adata, n_clusters, radius=radius, method=tool)
                    ...
                    current_labels = F.one_hot(torch.FloatTensor(self.adata.obs['domain']).long()-1,n_clusters).float().to(self.device)
                ...
                if epoch % 50==0:
                    ...
                    radius = 50
                    tool = 'mclust'
                    values = clustering(self.adata, n_clusters, radius=radius, method=tool)
                    old_type = self.adata.obs['domain'].values
