...
from GraphST import preprocess, get_feature
...
for i in files:
    ...
    #model = IST.GraphST(adata, device=device)

    # train model
    #adata = model.train()
    adata.obsm['emb'] = adata.obsm['feat']
    #adata.obsm['emb'] = torch.FloatTensor(adata.obsm['feat'].copy()).to(torch.device('cpu'))
